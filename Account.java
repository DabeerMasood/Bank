// Account.java
    ...
    public class Account implements Comparable<Account> {
        ...
        public int compareTo(Account x) {
            String namer = name;
            int amounter= amount;
            if(namer.equals(x.name)){
                if(amounter>0 && x.amount()<0){
                    return -1;
                }else if (amounter<0 && x.amount>0){
                    return 1;
                }else if (amounter<0 && x.amount()<0 && amounter<x.amount()){
                    return -1;
                }else if (amounter<0 && x.amount()<0){
                return 1;
                }else{
            ...
            }else if (namer.compareTo(x.name)<0){
            return -1;
            }else{
            return 1;
            }
